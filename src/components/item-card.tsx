...
export function ItemCard({ item, onClick, itemIndex, height, interactive = true }: ItemCardProps) {
  ...
  const toneClass = wouldCauseDebt
    ...
    : isEntry
      ...
      : isIncome
        ? "border-income/40 bg-income/5 text-income border-dashed"
        : "border-spend/35 bg-spend/4 text-spend border-dashed"
    ...
    >
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1 min-w-0 flex-1">
          ...
          <span className="sr-only">{isEntry ? "recorded" : "planned"}</span>
        </div>
